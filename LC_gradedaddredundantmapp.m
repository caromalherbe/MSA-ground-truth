...
scorebin=ones(n_pat,max_pairs);


for k = 1:n_groundT 
    for h = 1:n_pat
        score(h,k) = (lesion_map(h,pairs(1,k))+lesion_map(h,pairs(2,k)))/2;
         if data_binary(h,pairs(1,k))== 0 && data_binary(h,pairs(2,k)) == 0  
                scorebin(h,k) = 0;
            end
...
save('LC_doublesourcegradedadditionredundantMAPP.mat')
