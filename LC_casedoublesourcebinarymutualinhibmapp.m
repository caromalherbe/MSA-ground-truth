...
score = zeros(n_pat,max_pairs); %50 GT models in this case, default is all zeros, which means bad performance and 1 means good
    ...
    accuracy_L1o(k) = (count/p)*100;
        ...
        for m = 1:n_reg %selezione la regione di cui devo calcolare il rmse
            ...
            for in = 1:p
                ...
                e_mapp(in)=predicted_label_mappboot-scoreboot(p-in+1,1); %predicted-real
            end
            rmseboot = sqrt((sum(e_mapp.^2))/p);
            contr_mappboot(nb_boot,m,k) = rmseboot-rmse_total(k);
...
[Yessaisorted,Iessaisorted]=sort(contr_mappboot,2);
...
for i =1:n_groundT
    sommaboot = sommaboot + abs(matrixboot(40,i)) + abs(matrixboot(41,i));
end
...
matrix_nboot(:,1) = matrixboot([40 41],1);
matrix_nboot(:,2) = matrixboot([40 41],2);
...
save('LC_doublesourcebinarymutualinhibMAPP.mat')
