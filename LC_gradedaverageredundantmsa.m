...
for k = 1:n_groundT
    ...
    for h = 1:n_pat
             ...
             score(h,k) = (lesion_map(h,first)+lesion_map(h,second))/2;
              if data_binary(h,first)== 0 && data_binary(h,second) == 0  %try with double lesions
                scorebin(h,k) = 0;
            end
...
save('LC_doublesourcegradedaverageredundancyMSA.mat')
