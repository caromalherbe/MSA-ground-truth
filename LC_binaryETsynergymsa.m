...
for k = 1:n_groundT %1 groundtruth model at a time
    ...
    bigmat=[data_binary, score(:,k)]; %matrice avec les pateints en ligne et en colone, les regions puis le score clinique
...
%%%remove nan...
m_cvEpbootest(all(isnan(m_cvEpbootest),2),:)=[];
s_cvEpbootest(all(isnan(s_cvEpbootest),2),:)=[];
...
save('LCdoublesourcesynergyETbinaryMSA.mat')
