...
score = zeros(n_pat,max_pairs); %default is all zeros, which means bad performance and 1 means good
...
for k = 1:n_groundT
        ...
        for h = 1:n_pat
            if  lesion_map(h,first)==0 && lesion_map(h,second)==0
                score(h,k) = 1;
               
            else score(h,k) = 2*(lesion_map(h,first)*lesion_map(h,second))/((lesion_map(h,second)*(lesion_map(h,second))+(lesion_map(h,first)*lesion_map(h,first))));
                
            end
            if (data_binary(h,first)== 1 && data_binary(h,second) == 1) || (data_binary(h,first)== 0 && data_binary(h,second) == 0)   
                scorebin(h,k) = 1;
            end
...
accuracy_L1o = zeros(1,n_groundT); %I have one accuracy for every groundtruth
...
for k = 1:n_groundT %1 groundtruth model at a time
   ...
   [sortdata, sortid]=sort(scorebin(:,k))
    ...
    lesion_mapre=double(lesion_maptmp);%(1:300,:);
    ...
    for samplbootest=1:nb_bootest %pour tous les echantillons de bootstrap
        ...
        clear nouvvect
          ...
          for pat=1:200 %on cree un vecteur sur les aptients avec remise pour avoir un echantillon de bootstrap de taille p (nb de aptients)
    ...
    for m = 1:(n_reg)
       ...
       if  m == 1 | m == 41
           ...
           elseif  m ~= 1 & m ~= 41
            ...
            else errboot = errboot + abs(matrixboot(m,k));
...
save('LC_doublesourcegradedmutualinhibMSA.mat')
